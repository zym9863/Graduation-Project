...
from src.models.cross_modal_attention import CrossModalCrossAttentionLayer
...
class CrossModalNewsEncoder(nn.Module):
    def __init__(
        ...
        self.category_embedding = nn.Embedding(num_categories, category_emb_dim, padding_idx=0)
        self.subcategory_embedding = nn.Embedding(num_subcategories, category_emb_dim, padding_idx=0)
        self.cross_modal_fusion = CrossModalCrossAttentionLayer(
    ...
    def forward(
        ...
        fused = self.cross_modal_fusion(text_emb, image_emb, news_value)
        category_emb = self.category_embedding(category_ids)
        subcategory_emb = self.subcategory_embedding(subcategory_ids)
        return self.output_projection(torch.cat([fused, category_emb, subcategory_emb], dim=-1))
...
class TextImageNewsEncoder(nn.Module):
    def __init__(
        self,
        siglip_dim: int,
        category_emb_dim: int,
        ...
        self.category_embedding = nn.Embedding(num_categories, category_emb_dim, padding_idx=0)
        self.subcategory_embedding = nn.Embedding(num_subcategories, category_emb_dim, padding_idx=0)
        input_dim = siglip_dim * 2 + category_emb_dim * 2
        self.projection = nn.Sequential(
            nn.Linear(input_dim, news_repr_dim),
            nn.ReLU(),
            nn.Dropout(dropout),
    ...
    def forward(
        ...
        category_emb = self.category_embedding(category_ids)
        subcategory_emb = self.subcategory_embedding(subcategory_ids)
        combined = torch.cat([text_emb, image_emb, category_emb, subcategory_emb], dim=-1)
        return self.projection(combined)


class TextValueNewsEncoder(nn.Module):
    def __init__(
        self,
        siglip_dim: int,
        news_value_dim: int,
        ...
        self.category_embedding = nn.Embedding(num_categories, category_emb_dim, padding_idx=0)
        self.subcategory_embedding = nn.Embedding(num_subcategories, category_emb_dim, padding_idx=0)
        input_dim = siglip_dim + news_value_dim + category_emb_dim * 2
        self.projection = nn.Sequential(
            nn.Linear(input_dim, news_repr_dim),
            nn.ReLU(),
            nn.Dropout(dropout),
    ...
    def forward(
        ...
        category_emb = self.category_embedding(category_ids)
        subcategory_emb = self.subcategory_embedding(subcategory_ids)
        combined = torch.cat([text_emb, news_value, category_emb, subcategory_emb], dim=-1)
        return self.projection(combined)


class TextImageValueNewsEncoder(ConcatNewsEncoder):
    """Alias of full-feature concat encoder for ablation naming consistency."""
